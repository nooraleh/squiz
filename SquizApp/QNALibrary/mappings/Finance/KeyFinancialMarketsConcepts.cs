...
namespace QNALibrary.mappings.Finance
{
    ...
    {2, new Dictionary<string, string>() {
        ...
        {"a", @"ANS = 73 * (1 + (0.08 * Fraction(92, 365)))
    ...
    {3, new Dictionary<string, string>() {
        {"q", @"What is the 're-investment rate'?"},
        {"a", @"The re-investment rate is the rate at which interim interest
    ...
    {4, new Dictionary<string, string>() {
        {"q", @"Which major currency-denominated deposites are associated
		with the following days in year convention?
			1) 365
			2) 360"},
        {"a", @"1) 365 - sterling deposit
			2) 360 - US dollar deposit"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {5, new Dictionary<string, string>() {
        {"q", @"Consider the following formula which assumes:
    ...
    {6, new Dictionary<string, string>() {
        ...
        {"a", @"A nominal interest rate is an interest rate which does not take inflation into account.
    ...
    {7, new Dictionary<string, string>() {
        {"q", @"a) What is an equivalent interest rate?

		b) Given your answer to (a), what is an effective interest rate?"},
        {"a", @"a)	An equivalent interest rate is the rate which achieves the same total proceeds
    ...
    {8, new Dictionary<string, string>() {
        {"q", @"a) Compare money-market basis with bond basis?
    ...
    {9, new Dictionary<string, string>() {
        {"q", @"What is a:
			a) Rate of discount
			b) Discount factor"},
        {"a", @"a) A rate of discount is the interest rate which has been chosen for
    ...
    {10, new Dictionary<string, string>() {
        ...
        {"a", @"The net present value is the net total of several present values (arising from
    ...
    {11, new Dictionary<string, string>() {
        ...
        {"a", @"There are two definitions for the IRR:
    ...
    {12, new Dictionary<string, string>() {
        {"q", @"a) What is a time deposit?
		b) State a common type of time deposit?"},
        {"a", @"a) A time deposit is an interest-bearing bank deposit that has a specific
			maturity date.
    ...
    {13, new Dictionary<string, string>() {
        {"q", @"a) Define the discount rate as it refers to the US Treasury bill.
    ...
    {14, new Dictionary<string, string>() {
        {"q", @"Give examples of instruments quoted in:
			1) Discount rate basis
			2) Yield basis"},
        {"a", @"1) Example of discount rate basis:
			a) US T-Bills
    ...
    {15, new Dictionary<string, string>() {
        {"q", @"What is a discount instrument?"},
        {"a", @"A discount instrument is any one where the coupon is zero (assuming positive interest rates
		the market value will always be less than face value)."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {16, new Dictionary<string, string>() {
        {"q", @"Explain what is meant by the value date (of a money market transaction)."},
        {"a", @"Also known as the settlement date, the value date of a money market transaction is the
		date on which the transaction is consummated i.e. delivery takes place."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {17, new Dictionary<string, string>() {
        {"q", @"a) What is the standard value date for most currencies in the international foreign
		...
		b) Given your answer to (a) are value dates for forward settlements based on:
			...
			2) the value date"},
        {"a", @"a) The standard value date (i.e spot value date) is two working days after the transaction
			date (a.k.a ""T+2"").
    ...
    {18, new Dictionary<string, string>() {
        {"q", @"Explain what is meant by dealing 'end-end'."},
        {"a", @"If the spot value date is the last working day of the month,
    ...
    {19, new Dictionary<string, string>() {
        {"q", @"Explain what the 'modified following' convention is."},
        {"a", @"Suppose that:
    ...
    {20, new Dictionary<string, string>() {
        {"q", @"a) What is a zero-coupon yield?

		b) What does the spot yield curve show?

		c) What is bootstrapping?"},
        {"a", @"a) A zero-coupon yield is the actual or theoretical yield earned on an instrument
    ...
    {21, new Dictionary<string, string>() {
        {"q", @"What is the spot yield curve also known as?"},
        {"a", @"The zero-coupon yield curve."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {22, new Dictionary<string, string>() {
        {"q", @"What is the key difference between the 'money-weighted rate of return (MWRR)' and
		the 'time-weighted return (TWR)'?"},
        {"a", @"The MWRR considers the effects of cash in- and outflows, while the TWR excludes deposits/withdrawals
		in its calculation."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {23, new Dictionary<string, string>() {
        {"q", @"How is the money-weighted rate of return calculated?"},
        {"a", @"The MWRR is calculated by finding the rate of return that will set the present
    ...
    {24, new Dictionary<string, string>() {
        ...
        {"a", @"The consecutive periods are marked by the withdrawal of money from the fund
		or the addition of new money to it."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {25, new Dictionary<string, string>() {
        {"q", @"What is the difference between a deferred annuity and an annuity due?"},
        {"a", @"Deferred annuity:
    ...
    {26, new Dictionary<string, string>() {
        ...
        {"a", @"An annuity can be defined as either:
    ...
    {27, new Dictionary<string, string>() {
        {"q", @"In the context of financial markets, what is the mathematical definition
		of volatility?"},
        {"a", @"Volatility is the standard deviation, generally annuallised (i.e n_trading_days for year)
    ...
    {28, new Dictionary<string, string>() {
        ...
        {"a", @"VaR is the maximum potential loss which a bank expects to suffer on its
    ...
    {29, new Dictionary<string, string>() {
        ...
        {"a", @"This means that if Bank A's position remains unchanged, there is a 95%
    ...
    {30, new Dictionary<string, string>() {
        {"q", @"In the context of VaR, what is the 'observation period'?"},
        {"a", @"The observation period is the past period chosen to calculate the VaR,
    ...
    {31, new Dictionary<string, string>() {
        {"q", @"a) In the context of FX trading, what is a pip?
    ...
    {32, new Dictionary<string, string>() {
        {"q", @"Consider the following currency pair:
			USD/CAD

		Q: Which of the two is:
			1) the base currency?
			2) the quote (also known as 'variable') currency?"},
        {"a", @"The base current is the first currency stated in the pair so in this
    ...
    {33, new Dictionary<string, string>() {
        {"q", @"a) In the context of FX markets, what is meant by the term 'outrights'?
		b) What are outrights also known as in the context of FX markets?"},
        {"a", @"a) The term outrights is used to describe a type of transaction where
    ...
    {34, new Dictionary<string, string>() {
        {"q", @"As you know, the standard value date (i.e. spot value date) is two
    ...
    {35, new Dictionary<string, string>() {
        {"q", @"What is the difference between a 'short date forward' and
		a 'long-dated forward'?"},
        {"a", @"Short date forward:
    ...
    {36, new Dictionary<string, string>() {
        ...
        {"a", @"A spot transaction refers to an exchange of currencies at the prevailing market rate."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    ...
    {38, new Dictionary<string, string>() {
        {"q", @"What is Spot Next (S/N)?"},
        {"a", @"A term used in foreign currency trading, it denotes the delivery of
    ...
    {39, new Dictionary<string, string>() {
        {"q", @"In the foreign currency markets, state three interchangeable terms for the 
		'spot rate'."},
        {"a", @"1) Benchmark rate
    ...
    {40, new Dictionary<string, string>() {
        {"q", @"What do we call the difference between the spot rate and the forward rate?"},
        {"a", @"The basis."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {41, new Dictionary<string, string>() {
        {"q", @"What is a cross currency?"},
        {"a", @"A cross currency refers to a currency pair or transaction that doesn't involve the US dollar.
    ...
    {42, new Dictionary<string, string>() {
        {"q", @"What is meant by a high/low yield currency?"},
        {"a", @"A high yield currency is a currency where the home country of said currency
    ...
    {43, new Dictionary<string, string>() {
        ...
        {"a", @"Market depth refers to a market's ability to absorb relatively large market orders
		without significantly impacting the price of the security."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {44, new Dictionary<string, string>() {
        {"q", @"In a ""vanilla"" interest rate swap (i.e adjusted-rate for fixed-rate), who is the:
			1) receiver or seller
			2) payer"},
        {"a", @"1) The receiver or seller swaps the adjustable rate payments in exchange for fixed interest payments.
    ...
    {45, new Dictionary<string, string>() {
        {"q", @"Consider a plain vanilla interest rate swap, in which there are two parties:
    ...
    {46, new Dictionary<string, string>() {
        ...
        {"a", @"EUR/CAD = (1.0750USD / CAD) * (1.3400EUR / USD)
    ...
    {47, new Dictionary<string, string>() {
        {"q", @"Consider the following currency deal quote:
    ...
    {48, new Dictionary<string, string>() {
        {"q", @"In FOREX, what is meant by transaction risk?"},
        {"a", @"Transaction risk refers to the currency exchange risk associated with the time delay
		between entering into a trade or contract and then settling it."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {49, new Dictionary<string, string>() {
        {"q", @"How do currency swaps differ from interest rate swaps?"},
        {"a", @"They can also involve principal exchanges."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    ...
    {51, new Dictionary<string, string>() {
        ...
        {"a", @"An inflation swap is a derivatives contract in which:
			Party A
    ...
    {52, new Dictionary<string, string>() {
        ...
        {"a", @"A bullet repayment is a lump-sum repayment of the principal at maturity.
    ...
    {53, new Dictionary<string, string>() {
        {"q", @"What is a swap spread?"},
        {"a", @"A swap spread is the difference between the fixed component (swap rate) of a given
    ...
    {54, new Dictionary<string, string>() {
        {"q", @"What is the swap rate?"},
        {"a", @"The swap rate is the rate of the fixed leg of a swap as determined by its
    ...
    {55, new Dictionary<string, string>() {
        {"q", @"What is a reset date (in the context of financial derivatives such as interest rate swaps)?"},
        {"a", @"A reset date is the date on which the floating leg of the swap is recalculated
    ...
    {56, new Dictionary<string, string>() {
        ...
        {"a", @"a) False. However, the FOMC has tools at its deposal to influence to 
			influence the federal funds rate."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {57, new Dictionary<string, string>() {
        {"q", @"a) What is the Interest on Reserve Balances (IORB)?
        ...
        {"a", @"a) The rate of interest a bank (depository institution) gets on deposits,
    ...
    {59, new Dictionary<string, string>() {
        {"q", @"a) Provide a generally accepted definition for 'durable goods'
		b) Given your answer to (a), what are 'core durable good'?"},
        {"a", @"a) Durable goods are:
    ...
    {60, new Dictionary<string, string>() {
        {"q", @"a) What are capital goods?
		b) Give a few examples of capital goods."},
        {"a", @"a) Capital goods are goods that:
    ...
    {61, new Dictionary<string, string>() {
        {"q", @"a) What is a credit spread (AKA yield spread)?
		b) Other than 'yield spreads', state two other terms that credit spreads
			are interchangeable with."},
        {"a", @"a) A credit spread (ALKA yield spread) is the differece in yield between
    ...
    {62, new Dictionary<string, string>() {
        {"q", @"What is the difference between subordinated debt (a.k.a junior debt)
		and senior debt?"},
        {"a", @"They differ in priority in the event of bankruptcy or liquidation.
    ...
    {63, new Dictionary<string, string>() {
        {"q", @"Define 'Eurodollar Business Day'"},
        {"a", @"A Eurodollar Business Day is any day on which commercial banks are
    ...
    {64, new Dictionary<string, string>() {
        {"q", @"1) In a forward rate agreement (FRA) which of the long or short is:
    ...
    {65, new Dictionary<string, string>() {
        {"q", @"a) For which type of bonds will NEITHER the Macaulay duration NOR the modified
        ...
        {"a", @"a) Bonds with embedded options (e.g. callable and putable bonds)
