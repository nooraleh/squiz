...
{
    public partial class CSharpIntermediate : QNABase
    {
        public CSharpIntermediate()
        : base(title: "John Paul Mueller's C# All-in-One for Dummies", category: QNACategory.CSharp, qnaMapping: qnaMapping_)
        { }

        public override string ToString()
        {
            return "CSharpIntermediate";
        }

        static Dictionary<int, Dictionary<string, string>> qnaMapping_ = new Dictionary<int, Dictionary<string, string>>() {
    {1, new Dictionary<string, string>() {
        {"q", @"You can divide delegates used specifically as function pointers
    ...
    {2, new Dictionary<string, string>() {
        {"q", @"Consider the snippet (Form1.cs):
            public partial class Form1 : Form 

        What does the keyword `partial` mean?"},
        {"a", @"The partial keyword indicates that this file is only part of the full class.
    ...
    {4, new Dictionary<string, string>() {
        {"q", @"Consider the following two lines:
        ...
        {"a", @"i)  Discard parameters
        ii) That you don't intend on using those parameters"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {5, new Dictionary<string, string>() {
        {"q", @"In C#, when you have a class on which interesting events arise, how can you advertise
        the availability of notifications to any classes that may have an interest in knowing
        about such events?"},
        {"a", @"By providing a delegate and a coresponding event."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {6, new Dictionary<string, string>() {
        {"q", @"What are the two event-related delegate types for .NET handles for you?"},
        {"a", @"1) EventHandler
    ...
    {7, new Dictionary<string, string>() {
        {"q", @"When should you use each of the following:
            1) Events
            2) Delegates
            3) Lambdas"},
        {"a", @"1) Events:
    ...
    {8, new Dictionary<string, string>() {
        {"q", @"i)  What is a project file?
        ii) What are the extensions for 
            a) C# project files
            b) C++ project files
        iii) What is the term for a set of project files?"},
        {"a", @"i)  A project file contains the instructions about which files 
    ...
    {9, new Dictionary<string, string>() {
        {"q", @"i)  C# 10.0 introduced the `global using` statement. Outline its usage.
    ...
    {11, new Dictionary<string, string>() {
        {"q", @"What are the two basic assembly types that c# can produce?"},
        {"a", @"1) Executable (.exe)
        2) Class library (.dll)"},
        {"snippetQ", @""},
        {"snippetA", @""},
    ...
    {12, new Dictionary<string, string>() {
        {"q", @"i)  What does CLR stand for?
        ii) What the CLR do?"},
        {"a", @"i)  Common language runtime
        ii) 
    ...
    {13, new Dictionary<string, string>() {
        {"q", @"i)  Before becoming a .dll or a .exe, what does C# compile your source into?
        iI) What is one benefit of your answer to (i)"},
        {"a", @"i)  C# compiles your source code into Common Intermediate Language (also [C]IL)
        ii) CIL can be used to interact with other programming languages, for example C++."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {14, new Dictionary<string, string>() {
        {"q", @"Consider an executable with takes arguments such as files from the command line.
        What is an alternative approach to running an .exe via the command line?"},
        {"a", @"Simply drag the file into the .exe from Windows File Explorer."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {15, new Dictionary<string, string>() {
        {"q", @"If you have a Visual Studio solution which contains more than .exe project,
        what must you do?"},
        {"a", @"You must specify to Visual Studio which .exe project is the 'startup project' i.e
        the one which runs from the Debug menu."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {16, new Dictionary<string, string>() {
        {"q", @"What distinguishing a project that compiles to a .exe (executable) or to
        a .dll (class library)?"},
        {"a", @"The presence of a `Main()` method."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {17, new Dictionary<string, string>() {
        {"q", @"What is the most private access modifier that you should use
    ...
    {18, new Dictionary<string, string>() {
        {"q", @"i)  How should you approach exceptions in class library code.
    ...
    {19, new Dictionary<string, string>() {
        {"q", @"State two benefits of using namespaces?"},
        {"a", @"1) Putting related classes in one scope.
    ...
    {20, new Dictionary<string, string>() {
        ...
        {"a", @"False. A namespace CAN multiple assemblies."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {21, new Dictionary<string, string>() {
        {"q", @"What happens if your classes are not wrapped in a namespace?"},
        {"a", @"C# puts the in the 'global namespace', which is the base (unnamed)
    ...
    {23, new Dictionary<string, string>() {
        {"q", @"Besides classes, state four other types that namspaces can contain."},
        {"a", @"1) delegate
        2) enum
        3) interface
        4) struct"},
    ...
    {26, new Dictionary<string, string>() {
        {"q", @"i)  What does the `new` modifier keyword do?
        ii) Give a snippet example to illustrate your answer to (i)"},
        {"a", @"i)  The `new` keyword explicitly hides a member that is inherited
            from a base class.
        ii)
        Snippet:
                public class Base
    ...
    {27, new Dictionary<string, string>() {
        {"q", @"What does a partial method do?"},
        {"a", @"A partial method allows you to split a method into a signature part
        and potentially an implementation part."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {28, new Dictionary<string, string>() {
        {"q", @"i)  What are optional parameters in C#?
        ii) Let:
        ...
        {"a", @"i)  Optional parameters are parameters that have a default value right in
    ...
    {29, new Dictionary<string, string>() {
        {"q", @"What is the `sealed` modifier keyword used for?"},
        {"a", @"The `sealed` keyword is used to stop inheriting the particular class from
		other classes. It is essentially `final` in C++."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {30, new Dictionary<string, string>() {
        {"q", @"What does the `in` parameter modifier do?"},
        {"a", @"The `in` keyword:
    ...
    {31, new Dictionary<string, string>() {
        ...
        {"a", @"int answer = addit(z: 3, y: 7, v: 4);"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {32, new Dictionary<string, string>() {
        {"q", @"i)  What are output parameters?
		ii) Which parameter modifier do we use to specify output parameters?"},
        {"a", @"i)  Output parameters are parameters in the method signature that actually
    ...
    {33, new Dictionary<string, string>() {
        {"q", @"i)  What does the `nameof` expression do?
		ii) Give a simple example of its usage."},
        {"a", @"i)  The nameof expression produces the name of a variable, type, or member
    ...
    {34, new Dictionary<string, string>() {
        {"q", @"i)   State the character used for the verbatim identifier.
    ...
    {36, new Dictionary<string, string>() {
        {"q", @"What is the keyword used to create a structure?"},
        {"a", @"struct"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {37, new Dictionary<string, string>() {
        {"q", @"State which of the following are reference types or value types:
    ...
    {39, new Dictionary<string, string>() {
        {"q", @"Why does an array of references incur a larger penalty in resource use/
		allocation time/deallocation time than an array of values?"},
        {"a", @"Because an array of references contains just pointers to the individual objects.
    ...
    {40, new Dictionary<string, string>() {
        {"q", @"What is a best practice for structure members? Explain the rationale."},
        {"a", @"Best practice: Keep structures limited to other value types such as `int` and `double` where possible,
    ...
    {42, new Dictionary<string, string>() {
        {"q", @"What are two benefits of using read-only structures?"},
        {"a", @"1)  Performance
		2)  Thread safety"},
        {"snippetQ", @""},
        {"snippetA", @""},
    ...
    {43, new Dictionary<string, string>() {
        ...
        {"a", @"public readonly struct ReadOnlyData
			{
    ...
    {44, new Dictionary<string, string>() {
        {"q", @"State as many limitations of using a reference structure as you can."},
        {"a", @"1) No array element support
    ...
    {45, new Dictionary<string, string>() {
        {"q", @"What is the main readon to work with structures in most cases?"},
        {"a", @"To create records that contain custom data. You use these
    ...
    {47, new Dictionary<string, string>() {
        {"q", @"Outline some of the features of the `record` type."},
        {"a", @"1) Ability to define immutable properties
    ...
    {49, new Dictionary<string, string>() {
        {"q", @"As you know, `record` types use value equality as opposed to reference equality.
		Let:
        ...
        {"a", @"ReferenceEquals(record1, record2);"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {50, new Dictionary<string, string>() {
        {"q", @"a) What is a backing field (a.k.a backing store)?
		b) Give an example of its usage"},
        {"a", @"a) A backing field is a private field that stores the data exposed
    ...
    {52, new Dictionary<string, string>() {
        {"q", @"Give a brief outline of the meaning of the terms:
			1) authentication
			2) authorisation"},
        {"a", @"Authentication:
    ...
    {53, new Dictionary<string, string>() {
        {"q", @"In the context of threat modelling:
    ...
    {54, new Dictionary<string, string>() {
        {"q", @"What is the primary role of the System.Data namespace?"},
        {"a", @"Getting information out of and into a database."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {55, new Dictionary<string, string>() {
        {"q", @"i)  What does LINQ stand for?
		ii) What does LINQ enable you to do?"},
        {"a", @"i)  LINQ stands for 'language integrated query'
    ...
    {56, new Dictionary<string, string>() {
        {"q", @"What is the black box problem?"},
        {"a", @"The black box problem is that of having a development environment
		do some things for you over which you have no control."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {57, new Dictionary<string, string>() {
        {"q", @"Give a brief overview of the following C# data containers:
			1) DataSet
			2) DataTable
			3) DataRow
			4) DataView
    ...
    {58, new Dictionary<string, string>() {
        {"q", @"There are two SQL Server database files, for each:
    ...
    {60, new Dictionary<string, string>() {
        {"q", @"What are the two kinds of output that programs can generate?"},
        {"a", @"1) Binary
		2) Text-based"},
        {"snippetQ", @""},
        {"snippetA", @""},
    ...
    {61, new Dictionary<string, string>() {
        {"q", @"Explain the purpose of the following file modes:

			1) FileMode.CreateNew
			2) FileMode.Append
			3) FileMode.Create"},
        {"a", @"1) FileMode.CreateNew
    ...
    {62, new Dictionary<string, string>() {
        {"q", @"State the three FileAccess possibilities:"},
        {"a", @"1) FileAccess.Read
		2) FileAccess.Write
    ...
    {64, new Dictionary<string, string>() {
        {"q", @"1) What is the usual way to do stream writing in C#

			Hint: The answer has parallels with Python's `with` context manager."},
        {"a", @"1) The usual way is with a `using` statement.
    ...
    {65, new Dictionary<string, string>() {
        {"q", @"Consider the following template for `using` resource management idiom:
    ...
    {66, new Dictionary<string, string>() {
        {"q", @"Consider System.Net.Cache 

			What does a cache provide?"},
        {"a", @"A cache provides the means to improve application performance by storing commonly
		used resources."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {67, new Dictionary<string, string>() {
        {"q", @"In the context of System.Net, what does MIME stand for?"},
        {"a", @"MIME = Multipurpose Internet Mail Exchange"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {68, new Dictionary<string, string>() {
        {"q", @"1) Which C# keyword has a similar purpose to C++'s 'auto'? 
    ...
    {69, new Dictionary<string, string>() {
        {"q", @"What does the type keyword `dynamic` do?"},
        {"a", @"The `dynamic` keyword bypasses static type checking."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {70, new Dictionary<string, string>() {
        ...
        {"a", @"Compile time error. Specifically:
    ...
    {73, new Dictionary<string, string>() {
        {"q", @"State some of the trade-offs to consider when using the `static` keyword."},
        {"a", @"1) You can now access static members, including constants, of an enclosing scope.
    ...
    {74, new Dictionary<string, string>() {
        {"q", @"Explain the difference between the two console application project types
    ...
    {75, new Dictionary<string, string>() {
        {"q", @"What is the purpose of a 'Service' C# Visual Studio project?"},
        {"a", @"Purpose:
    ...
    {76, new Dictionary<string, string>() {
        {"q", @"When would you want to deselect 'Place solution and project in the same directory'
		when creating a new Visual Studio project?"},
        {"a", @"- When you want to add multiple projects to a single solution
    ...
    {77, new Dictionary<string, string>() {
        {"q", @"In which Visual Studio pane will a `Debug.Write()` statement go?"},
        {"a", @"The 'Output' pane."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {78, new Dictionary<string, string>() {
        {"q", @"a) What is a good pane for interactive debugging in Visual Studio?
		b) How do you access it if it's not there?"},
        {"a", @"a) The 'immediate' pane
		b) Debug->Windows->Immediate"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {79, new Dictionary<string, string>() {
        {"q", @"a) In which pane can you see all the breakpoints set up in throughout a given solution?
		b) How would you access it if it's not already there?"},
        {"a", @"a) 'Breakpoints' pane
		b) Debug->Windows->Breakpoints"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {80, new Dictionary<string, string>() {
        {"q", @"In terms of Visual Studio breakpoint options, outline the purpose of the following:
			1) Hit count
			2) Filter
			3) Actions"},
        {"a", @"1) Hit count:
    ...
    {81, new Dictionary<string, string>() {
        {"q", @"1) Explain how you can leverage the 'Task List' window?
    ...
    {82, new Dictionary<string, string>() {
        {"q", @"Consider the option available to you in the Visual Studio
		'Build' dropdown:
