...
{
    public partial class CPPSTL : QNABase 
    {
        public CPPSTL()
        : base(title: "C++ Standard Library", category: QNACategory.CPP, qnaMapping: qnaMapping_)
        { }
    ...
    {1, new Dictionary<string, string>() {
        {"q", @"Which type_trait must a class satisfy in order for an instance
    ...
    {2, new Dictionary<string, string>() {
        {"q", @"a) State the standalone function used for Euler exponentiation?
		b) Use your answer to (a) as well as other code to represent:
			 e** i * pi"},
        {"a", @"a) std::exp() - so std::exp(1) is e**1
		b)
		Snippet:
    ...
    {3, new Dictionary<string, string>() {
        {"q", @"What is std::reference_wrapper<T> in a nutshell?"},
        {"a", @"A std::reference_wrapper<T> is an object that:
    ...
    {4, new Dictionary<string, string>() {
        {"q", @"a) What does std::enable_shared_from_this allow for
		b) What is a pre-requisite to using std::enable_shared_from_this"},
        {"a", @"a) It enables you to get a valid std::shared_ptr instance to this,
    ...
    {6, new Dictionary<string, string>() {
        {"q", @"What does std::variant<T...>::index return?"},
        {"a", @"A zero-based index of the alternative that is currently held by the variant.
    ...
    {7, new Dictionary<string, string>() {
        {"q", @"State two cases in which you'd want to use <variant>'s std::monostate?"},
        {"a", @"1)	For use in a variant of non-default-constructible types.
    ...
    {8, new Dictionary<string, string>() {
        {"q", @"a) What is the purpose of std::variant<T...>::valueless_by_exception() ?
		b) What is the purpose of <variant>'s std::variant_npos?"},
        {"a", @"a) std::variant<T...>::valueless_by_exception returns false if and only if the
    ...
    {10, new Dictionary<string, string>() {
        ...
        {"a", @"auto holding_string = std::holds_alternative<std::string>(__variant);"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {11, new Dictionary<string, string>() {
        {"q", @"a) What is the purpose of std::visit?
		b) State any constraints there may be on std::visit?"},
        {"a", @"a) The purpose of std::visit is to pass a callback to be invoke on the underlying
    ...
    {12, new Dictionary<string, string>() {
        {"q", @"When would we favour <utility>'s std::as_const over const_cast?"},
        {"a", @"When you want to access the const member functions of an object, and
		not accidentally remove const if the object is already const."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {13, new Dictionary<string, string>() {
        {"q", @"a) Which C++20 class provides information regarding source code, including file names, line numbers,
		the calling function etc..
		b) In which header file can I find your answer to (a)"},
        {"a", @"a) std::source_location
		b) <source_location>"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {14, new Dictionary<string, string>() {
		...
		compile time or runtime when we are in:
			a) C++20
			b) C++23"},
        {"a", @"a) <type_trait>'s std::is_constant_evaluated
    ...
    {16, new Dictionary<string, string>() {
        {"q", @"Explain the motivation behind std::span (particular what it intends to replace)
    ...
    {17, new Dictionary<string, string>() {
        {"q", @"What is the formal definition of an std::ranges::view ?"},
        {"a", @"A `view` is a range which:
    ...
    {18, new Dictionary<string, string>() {
        {"q", @"What is <functional>'s std::identity used for?"},
        {"a", @"std::identity is simply a function object type whose `operator()` returns
		its argument unchanges."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {19, new Dictionary<string, string>() {
        {"q", @"What does the concept `std::semiregular` specify?"},
        {"a", @"The concept `std::semiregular` specifies a type T that is both:
			1) copyable and;
    ...
    {20, new Dictionary<string, string>() {
        {"q", @"What is the purpose/use case of std::declval?"},
        {"a", @"Converts any type T into a reference type, making it possible to use member functions
    ...
    {21, new Dictionary<string, string>() {
        {"q", @"In what scenario would you prefer using <memory>'s `std::addressof` as opposed to
		simple calling `&` on a (non-rvalue) object?"},
        {"a", @"When you're using user-defined types/third-party libraries and are unsure if the 
    ...
    {23, new Dictionary<string, string>() {
        {"q", @"a) What function would you use to determine if a given floating-point number
			is negative?
    ...
    {25, new Dictionary<string, string>() {
        {"q", @"1) Express a duration instance as:
    ...
    {26, new Dictionary<string, string>() {
        {"q", @"Give a brief explanation as to the purpose of <chrono>'s std::time_point?"},
        {"a", @"A std::time_point is:
    ...
    {27, new Dictionary<string, string>() {
        {"q", @"a) Which header file should I turn to to get implementation-dependent information
    ...
    {28, new Dictionary<string, string>() {
        {"q", @"What is the difference between <cstdint>'s:
			1) std::uint8_t, and;
			2) std::uint_fast8_t ?
    ...
    {29, new Dictionary<string, string>() {
        {"q", @"a) What is the purpose of C++23's `std::out_ptr` and `std::inout_ptr`? 
		b) When should you use `std::out_ptr` and when should you turn to `std::inout_ptr`?"},
        {"a", @"a) std::out_ptr and std::inout_ptr exist for compatibility between C++ smart pointers
    ...
    {30, new Dictionary<string, string>() {
        {"q", @"What does `std::thread::hardware_concurrency` return?"},
        {"a", @"It returns the number of threads supported by the current
		implementation's hardware can suport."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {31, new Dictionary<string, string>() {
        {"q", @"In the <concepts> context, when constraining a type T with `requires`.
