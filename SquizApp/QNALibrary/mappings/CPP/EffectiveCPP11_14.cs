...
{
    public partial class EffectiveCPP11_14 : QNABase
    {
        public EffectiveCPP11_14()
        : base(title: "Scott Meyer's Effective Modern C++", category: QNACategory.CPP, qnaMapping: qnaMapping_)
        { }

        public override string ToString()
        {
            return "EffectiveCPP11_14";
        }

        static Dictionary<int, Dictionary<string, string>> qnaMapping_ = new Dictionary<int, Dictionary<string, string>>() {
    {1, new Dictionary<string, string>() {
        {"q", @"In the context of exception safety, what is meant when a function is said 
		to offer:

			i)  the basic exception guarantee
			ii) the strong exception guarantee"},
        {"a", @"i) Basic guarantee - Assures function callers that even if an exception
    ...
    {2, new Dictionary<string, string>() {
        {"q", @"In the context of design-by-contract, what is meant by a program invariant?"},
        {"a", @"A program invariant is condition that must be satisfied during the whole execution
    ...
    {3, new Dictionary<string, string>() {
        ...
        {"a", @"1) T is `int`, param's type is `int&`
    ...
    {4, new Dictionary<string, string>() {
		...
		2) const int cx = x; f(cx);
		3) const int& rx = x; f(rx);"},
        {"a", @"1) T is `int`, param's type is `const int&`
    ...
    {5, new Dictionary<string, string>() {
		...
		in the following cases:
			...
			3) f(rx);
			4) f(27);"},
        {"a", @"1) x is lvalue, so: (i) T is int& (ii) param's type also int&
    ...
    {6, new Dictionary<string, string>() {
        {"q", @"Fill in the blank in the following statement:

			`const` and `volatile` qualifiers are ignored only for by-<_____> parameters"},
        {"a", @"ANS: value (pass-by-value parameters)"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {7, new Dictionary<string, string>() {
        ...
        {"a", @"i)  (1) array names (2) function names
			ii) unless they're used to initializze references"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {8, new Dictionary<string, string>() {
        {"q", @"i) In C++11, what is the only way in which `auto` type deduction and template
			type deduction differ. 
			
		ii) Give a concrete example of your answer to (i)"},
        {"a", @"i)  In the treatment of std::initializer_list
    ...
    {11, new Dictionary<string, string>() {
        {"q", @"Consider the following snippets:
    ...
    {12, new Dictionary<string, string>() {
        {"q", @"What is the main benefit (difference) of using <functional>'s std::function
		over employing function pointers?"},
        {"a", @"Function pointers can only point to functions. However, std::function
		objects can refer to any callable object."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {13, new Dictionary<string, string>() {
        {"q", @"std::vector<T>::operator[] (assuming access is valid)
		returns a reference to an element of type T for all T except for which
		data type?"},
        {"a", @"Except for bool. std::vector<bool> returns std::vector<bool>::reference
    ...
    {14, new Dictionary<string, string>() {
        {"q", @"i)  What is a proxy class?
		ii) Give a concrete example of a proxy class?"},
        {"a", @"i) A proxy class is a class that exists for the purpose of emulating
    ...
    {16, new Dictionary<string, string>() {
        ...
        {"a", @"i) Implicit narrowing conversion
    ...
    {17, new Dictionary<string, string>() {
        ...
        {"a", @"ANS: parameterless constructor
    ...
    {18, new Dictionary<string, string>() {
        {"q", @"What is a compelling technical reason for preferring 'using' aliases
		declarations over `typedef`s?"},
        {"a", @"In C++11, alias declarations may be templatized (alias templates) while
		`typedef`s cannot.
    ...
    {19, new Dictionary<string, string>() {
        {"q", @"When C++ is given a choice between converting a float to an:
			1) int
			2) double
    ...
    {20, new Dictionary<string, string>() {
        ...
        {"a", @"All three statements are false."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {21, new Dictionary<string, string>() {
        ...
        {"a", @"False. This is because template specialization must be
		written at namespace scope, not class scope."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {22, new Dictionary<string, string>() {
        {"q", @"Let's talk class member function reference qualifiers.
        ...
        {"a", @"(1) this version of do_work() is called only
    ...
    {23, new Dictionary<string, string>() {
        {"q", @"Describe the object code ramifications of declaring a function `noexcept`?"},
        {"a", @"In a `noexcept` marked function:
    ...
    {24, new Dictionary<string, string>() {
        {"q", @"In the context of library interface design, what is meant by:
		a)
    ...
    {25, new Dictionary<string, string>() {
        {"q", @"Which of `std::atomic` or `std::mutex` is more appropriate for the following scenarios:
    ...
    {26, new Dictionary<string, string>() {
        {"q", @"What is the difference in code generation between:
			1) copy construction/assignment, AND;
			2) move construction/assignment"},
        {"a", @"The copy construction/assignment operations are independent.
    ...
    {27, new Dictionary<string, string>() {
        ...
        {"a", @"i) False, move operations will not be generated in that case.
    ...
    {29, new Dictionary<string, string>() {
        {"q", @"In C++11, move operations are (implicitly compiler) generated for
		classes only if what three things are true?"},
        {"a", @"1) No copy operations are declared in the class
    ...
    {30, new Dictionary<string, string>() {
        {"q", @"Why do polymorphic base classes normally have virtual destructors?"},
        {"a", @"Because if they don't, some operations (e.g the use of `delete`
    ...
    {31, new Dictionary<string, string>() {
        {"q", @"What is a common use case for std::unique_ptr ?"},
        {"a", @"A common use for std::unique_ptr is as a factory function return type
		for objects in a hierarchy."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {32, new Dictionary<string, string>() {
        {"q", @"Consider the following statements about the existence of the 
        ...
        {"a", @"1) Because they internally contain a raw pointer to the resource
    ...
    {33, new Dictionary<string, string>() {
        {"q", @"Which of the following smart pointers:
        ...
        {"a", @"ANS: (2) std::unique_ptr i.e line (2) is correct"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {34, new Dictionary<string, string>() {
        {"q", @"In the context of smart pointer memory management, what is the
		control block?"},
        {"a", @"The control block is a heap-allocated data structure that exists for each
    ...
    {35, new Dictionary<string, string>() {
        {"q", @"Why would constructing (not via make_shared) more than one
    ...
    {36, new Dictionary<string, string>() {
        {"q", @"i)  What would you use if you want a class managed by std::shared_ptrs
			to be able to safely create a std::shared_ptr from a `this` pointer?
		ii)	Write a code snippet to demonstrate its usage."},
        {"a", @"i) Use <memory>'s base class template `std::enable_shared_from_this<T>`
    ...
    {37, new Dictionary<string, string>() {
        {"q", @"i)  What does CRTP stand for?
		ii) Which base class template is most commonly associated with CRTP?"},
        {"a", @"i)  Curiously recurring template pattern - CRTP
    ...
    {38, new Dictionary<string, string>() {
        ...
        {"a", @"std::unique_ptr can work with C-style arrays, std::shared_ptr
		cannot.
    ...
    {40, new Dictionary<string, string>() {
        {"q", @"Which member function can you use to tests
		if a weak point no longer points to an object?"},
        {"a", @"std::weak_ptr::expired"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {41, new Dictionary<string, string>() {
        ...
        {"a", @"i) At runtime, the arguments for a function must be evaluated before
    ...
    {42, new Dictionary<string, string>() {
        {"q", @"Despite the overwhelming arguments of using std::make_*
		...
		favourable."},
        {"a", @"1) std::make_* doesn't permit for custom deleters - so if you were
    ...
    {44, new Dictionary<string, string>() {
        {"q", @"What does the Pimpl (pointer to implementation) idiom entail?"},
        {"a", @"The Pimpl idiom is a technique whereby you replace the data members
    ...
    {45, new Dictionary<string, string>() {
        {"q", @"What do we call a type that has been declared, but not defined?"},
        {"a", @"We call this type an 'incomplete type'."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {46, new Dictionary<string, string>() {
        {"q", @"Give a brief description of what the functions
			i)  std::move
			ii) std::forward
		Do in essence."},
        {"a", @"i)  std::move unconditionally casts its argument to an rvalue
    ...
    {47, new Dictionary<string, string>() {
        {"q", @"Why should you not declare objects `const` if you want to
		be able to move from them?"},
        {"a", @"Because move requests on `const` objects are silently transformed
    ...
    {48, new Dictionary<string, string>() {
        {"q", @"In which two context does using two &&'s lead an identifier being
		associated with a universal/forwarding reference -
		and not just an rvalue reference?"},
        {"a", @"Context 1: With template parameters 
    ...
    {49, new Dictionary<string, string>() {
        {"q", @"Consider the following snippet which allows `param` to be a 
        ...
        {"a", @"1) Wrapping `T` in a sequential container:
    ...
    {51, new Dictionary<string, string>() {
        ...
        {"a", @"i)	1) the type of the local object is the same as that returned by the
				function.
    ...
    {52, new Dictionary<string, string>() {
        {"q", @"Why is overloading a function that takes a universal/forwarding reference
		a bad idea?
        ...
        {"a", @"Because the universal reference function will instantiate far more
    ...
    {53, new Dictionary<string, string>() {
        {"q", @"Explain what the following type traits do:
        ...
        {"a", @"1) Determines whether a type is integral (e.g. std::size_t, int, long unsigned <- True)
    ...
    {54, new Dictionary<string, string>() {
        {"q", @"i) Name a strategy for getting around overloading perfect forwarding
			functions.
    ...
    {55, new Dictionary<string, string>() {
        {"q", @"What is the purpose of std::enable_if?"},
        {"a", @"std::enable_if:
    ...
    {56, new Dictionary<string, string>() {
        {"q", @"In C++14, which type trait can we use to remove any references and CV-qualifiers?"},
        {"a", @"std::decay_t<T>"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {57, new Dictionary<string, string>() {
        {"q", @"Consider <type_trait>'s std::is_base_of and complete the following sentence.

		std::is_base_of<A, B>::value is true if ....."},
        {"a", @".... if B is derived from A (or A is a base of B)"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {58, new Dictionary<string, string>() {
        {"q", @"i) Which <type_trait> can we use to determine whether a type A is constructible
			from a type B?
    ...
    {59, new Dictionary<string, string>() {
        {"q", @"In the context of lambda expressions, what is:
	   
			i) a closure?
			ii) a closure class?"},
        {"a", @"i)  A closure is a runtime object created by a lambda.
    ...
    {60, new Dictionary<string, string>() {
        {"q", @"State whether the following exist during compilation or during runtime:

			1) lambdas
			2) closure classes
			3) closures"},
        {"a", @"1) compilation time
			2) compilation time
			3) runtime"},
        {"snippetQ", @""},
        {"snippetA", @""},
    ...
    {61, new Dictionary<string, string>() {
        {"q", @"Consider the following snippet and (i) determine what is wrong with (1):
        ...
        {"a", @"(1) uses the default-by-reference capture mode.
    ...
    {62, new Dictionary<string, string>() {
        {"q", @"i) What are init captures used for?
		ii) Give a code snippet exhibiting init capture usage."},
        {"a", @"i) Init captures are used for moving objects into closures.
    ...
    {64, new Dictionary<string, string>() {
        {"q", @"What's another name for C++14's init capture?"},
        {"a", @"Generalized lambda capture."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {65, new Dictionary<string, string>() {
        {"q", @"What is a C++14 generic lambda?"},
        {"a", @"A lambda which used at least one 'auto' parameter."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {66, new Dictionary<string, string>() {
        {"q", @"What is a good rule to remember with regards to reference
		collapsing?"},
        {"a", @"If either reference is an lvalue reference:
    ...
    {67, new Dictionary<string, string>() {
        {"q", @"State the four context in which reference collapsing
		occurs?"},
        {"a", @"1) Template instantiation
    ...
    {70, new Dictionary<string, string>() {
        {"q", @"What makes it possible for emplacement functions to outperform
		insertion functions?"},
        {"a", @"Essence:
