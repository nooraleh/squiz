...
{
    public partial class BoostAsio : QNABase
    {
        public BoostAsio()
        : base(title: "Anggoro's Boost.Asio C++ Network Programming", category: QNACategory.CPP, qnaMapping: qnaMapping_)
        { }
    ...
    {1, new Dictionary<string, string>() {
        {"q", @"What is the core object of the boost::asio namespace?"},
        {"a", @"The `io_service` object"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {2, new Dictionary<string, string>() {
        {"q", @"In the context of boost.asio, what is the I/O service?"},
        {"a", @"The I/O service is a channel that is used to:
    ...
    {3, new Dictionary<string, string>() {
        {"q", @"boost.asio context, what is understood as an `event`?"},
        {"a", @"An `event` is an action or occurrence detected by a program,
    ...
    {4, new Dictionary<string, string>() {
        {"q", @"What is the `boost::asio::io_service::work` object responsible for?"},
        {"a", @"It is responsible for telling the boost::asio::io_service object when the work
		starts and when the work has finished."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {5, new Dictionary<string, string>() {
        {"q", @"a) What is the boost::asio::io_service::poll member function used for?
		b) What is the difference between io_service::run() and io_service::poll()"},
        {"a", @"a) The poll() member function is used to run ready handlers until there are no
    ...
    {6, new Dictionary<string, string>() {
        {"q", @"a) What is the difference between the functionality of:
    ...
    {8, new Dictionary<string, string>() {
        {"q", @"What is understood as an 'event handler' in boost.asio?"},
        {"a", @"An 'event handler' is a function that is called when a particular event occurs on an
    ...
    {9, new Dictionary<string, string>() {
        ...
        {"a", @"An executor repesents an object that can execute functions or tasks.
    ...
    {10, new Dictionary<string, string>() {
        {"q", @"boost::asio::io_context::work is deprecated, consider the following
		snippet and replace it with the preferred calling code:

			boost::asio::io_context io_context;
			boost::asio::io_context::work work_guard{io_context};"},
        {"a", @"boost::asio::io_context::work is deprecated in favour of boost::asio::executor_work_guard
    ...
    {11, new Dictionary<string, string>() {
        {"q", @"`boost::asio::io_context::run` has an overload which takes no parameters.
			std::size_t boost::asio::io_context::run();

		What is the other overload and what is the other overload used for?"},
        {"a", @"The other overload of `boost::asio::io_context::run` takes a boost::system::error_code&
    ...
    {13, new Dictionary<string, string>() {
        {"q", @"What is the difference between a:
			1) Physical address
			2) Logical address"},
        {"a", @"Physical address (also known as MAC [media access control] address):
    ...
    {14, new Dictionary<string, string>() {
        {"q", @"What is the difference between a) shutting down a socket b) closing a socket?"},
        {"a", @"Socket shut down:
    ...
    {15, new Dictionary<string, string>() {
        {"q", @"a) What is the purpose of `boost::asio::ip::tcp::resolver` ?
		b) Give a concrete code example of your answer given in (a)"},
        {"a", @"a) The `boost::asio::ip::tcp::resolver` class is used to resolve
...
';
    ...
    {16, new Dictionary<string, string>() {
        {"q", @"What is the purpose of boost::asio::ip::tcp::acceptor::reuse_address ?"},
        {"a", @"`reuse_address` is a socket option to allow the socket in question to be bound
		to an address that is already in use."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {17, new Dictionary<string, string>() {
        {"q", @"What is `boost::asio::ip::tcp::socket::shutdown_both` used for?"},
        {"a", @"It is an option flag for shutting down both send and receive on
    ...
    {19, new Dictionary<string, string>() {
        ...
        {"a", @"A distributed application is one that:
    ...
    {20, new Dictionary<string, string>() {
        {"q", @"a) What is the 'point-to-point communication model'?
		b) Which of protocols 1) TCP or 2) UDP assume the point-to-point communication model?"},
        {"a", @"a)  Point-to-point communication model:
    ...
    {21, new Dictionary<string, string>() {
        ...
        {"a", @"An endpoint is a pair of values consisting of:
    ...
    {22, new Dictionary<string, string>() {
        {"q", @"Endpoints serve a goal for the client application as well as the server application.
		State the two."},
    ...
    {24, new Dictionary<string, string>() {
        {"q", @"True or false:
			...
			by default."},
        {"a", @"False, boost::asio::ip::address is an IP-protocol-version-agnostic class which can
		represent both IPv4 and IPv6."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {25, new Dictionary<string, string>() {
        {"q", @"Describe and distinguish the following terms in the context of the process of establishing
    ...
    {26, new Dictionary<string, string>() {
        {"q", @"In boost.asio, what does it mean to open a socket? I.e calling one of:
			1) boost::asio::ip::tcp::socket::open or
			2) boost::asio::ip::udp::socket::open ?"},
        {"a", @"In boost.asio, 'opening' a socket means associate the `socket` object with
    ...
    {27, new Dictionary<string, string>() {
        ...
        {"a", @"Because the udp protocol is connectionless (i.e does not establish connections),
    ...
    {29, new Dictionary<string, string>() {
        {"q", @"a) What is the purpose of `boost::asio::ip::tcp::resolver::query`?
		b) What are the constructor arguments?"},
        {"a", @"a) The `query` class abstracts a query to the DNS (domain name system).
    ...
    {31, new Dictionary<string, string>() {
        {"q", @"What is the name of the process of associating a socket with a particular endpoint?"},
        {"a", @"Name: Binding."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {32, new Dictionary<string, string>() {
        {"q", @"In the context of the point-to-point communication model, what are the implications
    ...
    {34, new Dictionary<string, string>() {
        ...
        {"a", @"Memory buffers are contiguous blocks of memory allocated in the process's address space
		used to store the data."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {35, new Dictionary<string, string>() {
        {"q", @"boost.asio supports two types of I/O operations:
    ...
    {36, new Dictionary<string, string>() {
        {"q", @"Why is the ability to cancel a previously initiated asynchronous operation important?"},
        {"a", @"It allows the application to state that the previously initiated operation is not relevant
    ...
    {37, new Dictionary<string, string>() {
        {"q", @"In boost.asio, a fixed-length buffer is represented by either of two classes,
		state them."},
        {"a", @"1) boost::asio::mutable_buffer
			- writable-buffer
    ...
    {38, new Dictionary<string, string>() {
		...
		True or false:
			...
			for their lifetimes."},
        {"a", @"False. They do not take ownership or control the underlying buffer resource
    ...
    {40, new Dictionary<string, string>() {
        {"q", @"Writing to a TCP is an output operation that is used to send data to the
		remote application connected to this socket.

		What is the most basic way to write to the socket in the boost.asio library?"},
        {"a", @"Using member function `boost::asio::ip::tcp::socket::write_some`"},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {41, new Dictionary<string, string>() {
        {"q", @"a) Which free function in the boost.asio library should you turn to in order
        ...
        {"a", @"a)  boost::asio::read_until, consider one of the overloads:
    ...
    {43, new Dictionary<string, string>() {
        {"q", @"What is the purpose of member function `boost::asio::ip::tcp::socket::cancel`?"},
        {"a", @"The purpose is to cancel all asynchronous operations (cancel, send and/or receive)
		associated with the instance of `boost::asio::ip::tcp::socket`."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {44, new Dictionary<string, string>() {
        {"q", @"Do the `boost::asio::ip::tcp::socket` class implement the RAII idiom? Explain."},
        {"a", @"Since the boost::asio::ip::tcp::socket class calls it's `close` member in the destructor,
    ...
    {45, new Dictionary<string, string>() {
        ...
        {"a", @"False. It is not posssible to restore the socket state to make it writable again."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {46, new Dictionary<string, string>() {
        {"q", @"A synchronous approach has some functional limitations which can make it unacceptable.
    ...
    {47, new Dictionary<string, string>() {
        {"q", @"What is the term for hybrid server applications that provide their services through both TCP
		and UDP?"},
        {"a", @"Multiprotocol servers."},
        {"snippetQ", @""},
        {"snippetA", @""},
    }},
    {48, new Dictionary<string, string>() {
        {"q", @"Give a brief summary of the following terms:
			1) iterative server
			2) parallel server
    ...
    {49, new Dictionary<string, string>() {
        {"q", @"Consider the following typedef's in the boost.asio library:
